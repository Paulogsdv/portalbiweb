...
export default function RankingSecoes({ empresa }) {
   ...
   async function rankingSecoes(data) {
      setLoading(true);
   ...
   return (
      ...
      >
         ...
         {secoes && secoes.length > 0 ? (
                     ...
                     {secoes?.map((sec, index) => (
                        <TR key={index}>
                           ...
                           <TD style={{ textAlign: 'right' }} color="#5e72e4">
                              <p>{sec?.faturamento > 0 ? `${formatValue(sec?.faturamento)}` : null}</p>
                           </TD>
                           <TD style={{ textAlign: 'right' }} color="#F65374">
                              <p>{sec?.custo > 0 ? `${formatValue(sec?.custo)}` : null}</p>
                           </TD>
                           <TD style={{ textAlign: 'right' }} color="#4CD59A">
                              <p>{formatValue(sec?.lucro)}</p>
                           </TD>
                        </TR>
                     ))}
                  </TBODY>
            ...
            <SPAN> Sem registros no ranking de seções neste período! </SPAN>
         )}
      </CARD>
   );
}

RankingSecoes.propTypes = {
